...
class SurrogateModelGPR(SurrogateModel):
    ...
    def estimate(
        ...
        *,
        ...
        noise_bounds: Bounds = (1e-5, 1e5),
        amplitude_bounds: Bounds = (1e-5, 1e5),
        length_scale_bounds: t.Union[Bounds, t.List[Bounds]] = (1e-3, 1e3),
        n_restarts_optimizer: int = 2,
        matern_nu: float = 5/2,
    ) -> 'SurrogateModelGPR':

        assert prior is None or isinstance(prior, SurrogateModelGPR)
        kernel = _make_new_kernel(
            prior=prior, n_dims=space.n_dims,
            amplitude=1.0, amplitude_bounds=amplitude_bounds,
            length_scale_bounds=length_scale_bounds,
            noise=1.0, noise_bounds=noise_bounds,
            matern_nu=matern_nu)

        ys_mean = np.mean(vec_ys)
        ys_min = np.min(vec_ys)

        mat_x_train = np.array([space.into_transformed(x) for x in mat_xs])
        vec_y_train = vec_ys - ys_min
        relax_alpha = 1e-10
        lml = fit_kernel(
    ...
    def from_kernel(
        ...
        *,
        ...
        if lml is None:
            neg_lml, _lml_grad = log_marginal_likelihood(
        ...
        try:
            _mat_k, mat_l, vec_alpha, _mat_k_grad = \
                _calculate_prediction_matrices(
    ...
    def predict_transformed_a(
        ...
        mat_k_inv = self.K_inv

        mat_k_trans = kernel(mat_x, self.X_train)
        vec_y_mean = mat_k_trans.dot(vec_alpha)
        vec_y_mean += self.ys_min  # undo normalization

        if return_std:
            # Compute variance of predictive distribution
            vec_y_var = kernel.diag(mat_x)
            vec_y_var -= np.einsum(
                "ki,kj,ij->k", mat_k_trans, mat_k_trans, mat_k_inv)

            # Check if any of the variances is negative because of
            # numerical issues. If yes: set the variance to 0.
            vec_y_var_negative = vec_y_var < 0
            if np.any(vec_y_var_negative):
                warnings.warn("Predicted variances smaller than 0. "
                              "Setting those variances to 0.")
                vec_y_var[vec_y_var_negative] = 0.0
            vec_y_std = np.sqrt(vec_y_var)
            return vec_y_mean, vec_y_std
...
def fit_kernel(
    ...
    def obj_func(theta: np.ndarray, eval_gradient: bool = True):
        lml, _lml_grad = log_marginal_likelihood(
...
def log_marginal_likelihood(
    ...
    *,
    kernel: Kernel,
    mat_x: np.ndarray,  # pylint: disable=invalid-name
    vec_y: np.ndarray,
    relax_alpha: float,
    ...
    try:
        mat_k, mat_l, vec_alpha, mat_k_gradient = \
            _calculate_prediction_matrices(
                mat_x, vec_y,
                kernel=kernel,
    ...
    log_likelihood = -0.5 * vec_y.dot(vec_alpha)
    log_likelihood -= np.log(np.diag(mat_l)).sum()
    log_likelihood -= mat_k.shape[0] / 2 * np.log(2 * np.pi)

    if eval_gradient:
        tmp = np.outer(vec_alpha, vec_alpha)
        tmp -= scipy.linalg.cho_solve((mat_l, True), np.eye(mat_k.shape[0]))
        # compute "0.5 * trace(tmp dot mat_k_gradient"
        # without constructing the full matrix
        # as only the diagonal is required
        log_likelihood_gradient = \
            0.5 * np.einsum('ij,ijk->k', tmp, mat_k_gradient)
...
def _calculate_prediction_matrices(
    mat_x: np.ndarray,
    vec_y: np.ndarray,
    *,
    kernel: Kernel,
    ...
    mat_l = scipy.linalg.cholesky(mat_k, lower=True)

    # solve the system "K alpha = y" for alpha,
    # based on the cholesky factorization L.
    vec_alpha = scipy.linalg.cho_solve((mat_l, True), vec_y)

    return mat_k, mat_l, vec_alpha, mat_k_gradient


def _make_new_kernel(
    *,
    prior: t.Optional[SurrogateModelGPR],
    n_dims: int,
    amplitude: float,
    amplitude_bounds: Bounds,
    length_scale_bounds: t.Union[Bounds, t.List[Bounds]],
    noise: float,
    noise_bounds: Bounds,
    matern_nu: float,
) -> Kernel:

    assert amplitude in ClosedInterval(*amplitude_bounds)
    ...
    assert noise in ClosedInterval(*noise_bounds)

    if prior is not None:
        return clone(prior.kernel)

    amplitude_kernel = ConstantKernel(amplitude, amplitude_bounds)
    ...
    noise_kernel = WhiteKernel(noise, noise_bounds)

    return Sum(Product(amplitude_kernel,
                       main_kernel),
               noise_kernel)
